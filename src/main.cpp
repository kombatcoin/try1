...
uint256 hashGenesisBlock("0x91a49c57566f8e5e275467aedee07d8b4564bbd381e64f982929ca1cd0aecef6"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KombatKoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KombatKoin Signed Message:\n";
...
{
    // KombatKoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // KombatKoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // KombatKoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KombatKoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, kombatkoin - DarkGravity v2, written by Evan Duffield - evan@kombatkoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, kombatkoin - DarkGravity v3, written by Evan Duffield - evan@kombatkoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // KombatKoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // KombatKoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Temp 7.26.2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x4ca263f53f409fca35ac3d22af5b99c13eba563a5503eff9b99b8a41e9999ea4"));
		block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KombatKoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KombatKoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KombatKoinMiner(CWallet *pwallet)
{
    printf("KombatKoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kombatkoin-miner");
    ...
    try { loop {
        ...
        printf("Running KombatKoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KombatKoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KombatKoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - KOMBATKOIN **********
    Copyright 2014, KombatKoin Developers 
        eduffield - evan@kombatkoin.io
        InternetApe - kyle@kombatkoin.io
*/
