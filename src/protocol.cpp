...
// KombatKoin message start (switch from Litecoin's)
static unsigned char pchMessageStartLitecoin[4] = { 0xfb, 0xc0, 0xb6, 0xdb };
static unsigned char pchMessageStartKombatKoin[4] = { 0xbf, 0x0c, 0x6b, 0xbd };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
    if (fTestNet)
        ...
        memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartKombatKoin : pchMessageStartLitecoin, sizeof(pchMessageStartKombatKoin));
}
